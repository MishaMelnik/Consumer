...
import {fetchDetail, API} from '../../services';
...
export class CatalogItemDetail extends Component {
  ...
  componentDidMount() {
    const {productId, product_type} = this.props.route.params;

    fetchDetail(GETDETAIL[product_type], productId)
      .then(response => {
        this.setState({
          data: response,
    ...
    return (
      <ScrollView style={styles.detailContainer}>
        ...
        <View style={styles.containerColumn}>
          ...
          {data.link === '' || data.link === undefined || isLoading ? null : (
            <TouchableOpacity
              onPress={() =>
                Linking.canOpenURL(data.link).then(supported => {
                  if (supported) {
                    ...
                    return Linking.openURL('https://perfumsbar.com/index.php');
                  }
                })
              }>
              <View style={styles.catalogItemButton}>
                <Text style={{color: 'white', fontSize: 16}}>{t('Buy')}</Text>
              </View>
            </TouchableOpacity>
          )}
